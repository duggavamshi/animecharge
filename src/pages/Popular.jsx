...
const Popular = () => {
  ...
  const { responseData: popularData, loading: popularLoading, err: popularErr } = useFetch(`https://api.amvstr.me/api/v1/popular?p=${page}`);
  return (
    <>
      <Container>
        <div className="m-2">
          <h2 className="text-xl font-bold">Popular #{page}</h2>
        </div>
        {popularLoading ? (
          <Loader />
        ) : popularErr ? (
